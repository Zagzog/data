            ...
            for neighbor1 in list(self.layer1.neighbors(nodes2)):
                if self.node_states[neighbor1] == states[0]:
                    if self.ow_nodes[neighbor1] != 0:
                        r1 = random.random()
                        spread_probability = (2*self.alpha / (1 + math.exp(-self.h_para * abs(self.opinion[neighbor1])))) * 1.0
...
def run_dynamics(information_layer, opinion_layer, alpha_a, frontier_extreme_number, central_extreme_number,
                 omega_w, iteration_time, name, central_supporters_number, h_para):
    ...
    path1 = 'F:/paper-data/model-data-add/' + name + '/simulation data'
    for i in range(mm):
        folder = os.path.exists(path1 + '/number' + str(i))
        if not folder:
            os.makedirs(path1 + '/number' + str(i))
    source_node1 = source_list[0]
    ide = IoDynamics(G1, G2, source_node1, alpha, fen, cen, ow, csn, h_para)
    number_of_A = {}
    steady_opinion = {}
    ...
    for s1 in range(mm):
        ...
        b = 'F:/paper-data/model-data-add/' + name + '/simulation data/number' + str(s1) + '/'
    ...
    f = open('F:/paper-data/model-data-add/' + name + '/simulation data/probability of knowing news.txt', 'w')
    for keys1 in number_of_A:
        f.write(str(keys1) + ':' + str(number_of_A[keys1]) + '\n')
    f.close()
    g5 = open('F:/paper-data/model-data-add/' + name + '/simulation data/average_opinion.txt', 'w')
...
if __name__ == '__main__':
    ...
    # the results of ER-BA
    G1 = nx.read_edgelist('F:/论文/network data/ER1_10000')
    G2 = nx.read_edgelist('F:/论文/network data/BA2_10000')
    ...
    for i in h_list:
        for j in j_list:
            print(i, j)
            nnnn = 'ER-BA/h-' + str(i) + '/' + str(j) + '-central extreme supporter'
            run_dynamics(G1, G2, alpha1, Ns, Ns, t, m, nnnn, j, i)
