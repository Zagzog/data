...
def run_dynamics(information_layer, opinion_layer, alpha_a, frontier_extreme_number, central_extreme_number,
                 omega_w, iteration_time, name, central_supporters_number):
    ...
    path1 = 'E:/paper-data/model-data/' + name + '/simulation data'
    for i in range(mm):
        folder = os.path.exists(path1 + '/number' + str(i))
        if not folder:
            os.makedirs(path1 + '/number' + str(i))
    source_node1 = source_list[0]
    ide = IoDynamics(G1, G2, source_node1, alpha, fen, cen, ow, csn)
    number_of_A = {}
    steady_opinion = {}
    ...
    for s1 in range(mm):
        ...
        b = 'E:/paper-data/model-data/' + name + '/simulation data/number' + str(s1) + '/'
    ...
    f = open('E:/paper-data/model-data/' + name + '/simulation data/probability of knowing news.txt', 'w')
    for keys1 in number_of_A:
        f.write(str(keys1) + ':' + str(number_of_A[keys1]) + '\n')
    f.close()
    g5 = open('E:/paper-data/model-data/' + name + '/simulation data/average_opinion.txt', 'w')
...
if __name__ == '__main__':
    G1 = nx.read_edgelist('E:/paper-data/network data/cannes_2')
    G2 = nx.read_edgelist('E:/paper-data/network data/cannes_3')
    ...
    alpha1 = 0.05
    for j in range(11):
          nnnn = 'cannes/' + str(j) + '-central extreme supporter'
          run_dynamics(G1, G2, alpha1, Ns, Ns, t, m, nnnn, j)
